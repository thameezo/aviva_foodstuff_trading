  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="w-12 h-12 rounded-full bg-accent-dim text-accent flex items-center justify-center">
          <CategoryIcon category={category} />
        </span>
        <span className="text-xs manifest-number font-medium">{String(index).padStart(2, "0")}</span>
      </div>
      <h3 className="mt-5 font-sans tracking-tight text-2xl font-semibold text-ink">{name}</h3>
